...
| BePushy | bool | Boolean indicating if resizing one column/row beyond min/max should push into and resize subsequent rows/columns. |
...
| OverrideProportionalSize | bool | Boolean indicating if the proportional size of the columns/rows should be overriden when resizing, and set to absolute (pixel) sizes instead. |
...
| SetSizeOnDragEnded | bool | Boolean indicating if the size of the row/column should be set after the user releases the handle. |
| SplitMode | [SplitMode](../Types/SplitMode) | Sets if the grid should be split by columns, rows or both. |
| SplitterColor | [Color](http://docs.unity3d.com/ScriptReference/Color.html) | Color used by the grid splitter handles. |
| Thickness | [ElementSize](../Types/ElementSize) | The thickness of the grid splitter handles. |
