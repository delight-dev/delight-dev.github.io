...
| Breadth | [ElementSize](../Types/ElementSize) | The breadth of the slider. Corresponds to Height if horizontal and Width if vertical. |
| BubbleNotifyChildLayoutChanged | bool | Boolean indicating if parent always should be notified when the child changes layout. |
| CanSlide | bool | Boolean indicating if the user can interact with the slider. |
...
| Orientation | [ElementOrientation](../Types/ElementOrientation) | Orientation of the slider (horizontal or vertical). |
...
| SetValueOnDragEnded | bool | Boolean indicating that the slider value is set when the user releases the handle. |
| Steps | float | Specifies the number of steps there should be between min and max slider value. |
